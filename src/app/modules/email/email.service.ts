...
<head>
    ...
    <title>Techlight IT Institute</title>
    <style>
...
<body style="background-color: #f8fafc; padding: 40px 20px;">
    <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
        ...
        <div style="background: linear-gradient(135deg, #14b8a6 0%, #0d9488 100%); padding: 30px; text-align: center;">
            <h1 style="color: white; font-size: 28px; font-weight: bold; margin: 0;">Techlight IT Institute</h1>
...
const getWelcomeEmailTemplate = (firstName: string) => getEmailWrapper(`
    <div style="text-align: center; margin-bottom: 30px;">
        ...
        <h2 style="color: #1e293b; font-size: 24px; margin-bottom: 10px;">Welcome to Techlight IT Institute!</h2>
    ...
    <p style="color: #334155; font-size: 16px; line-height: 1.6; margin-bottom: 25px;">
        Thank you for joining Techlight IT Institute! You now have access to our premium collection of website templates and software solutions.
    </p>
    ...
    async sendWelcomeEmail(email: string, firstName: string): Promise<boolean> {
        try {
            await transporter.sendMail({
                from: `"Techlight IT Institute" <${config.email.from}>`,
                to: email,
                subject: '🎉 Welcome to Techlight IT Institute - Your Premium Template Journey Begins!',
    ...
    async sendPasswordResetEmail(email: string, firstName: string, resetToken: string): Promise<boolean> {
        try {
            ...
            await transporter.sendMail({
                from: `"Techlight IT Institute" <${config.email.from}>`,
                to: email,
                subject: '🔐 Reset Your Password - Techlight IT Institute',
